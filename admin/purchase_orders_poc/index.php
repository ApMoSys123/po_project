...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title"><b><u>POC Purchase Order Details</b></u></h3>
		<div class="card-tools">
		<?php if($_settings->userdata('type') == 1): ?>
		<a href="?page=purchase_orders_poc/manage_poc" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Add New</a>
		<?php endif; ?>
		...
		<a href="?page=purchase_orders_poc/manage_poc" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Add New</a>
		<?php endif; ?>
		<a href="http://localhost/purchase_order/admin/purchase_orders_poc/export.php" class="btn btn-flat btn-success"><span class="fas fa-paper-plane"></span> Export</a>
        ...
        <div class="container-fluid">
			<table id="purchaseprojectfc" class="table table-hover table-striped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * FROM `poc_po_list`");
						while($row = $qry->fetch_assoc()):
							$row['item_count'] = $conn->query("SELECT * FROM order_items where po_id = '{$row['id']}'")->num_rows;
							$row['total_amount'] = $conn->query("SELECT unit_price as total FROM poc_po_list where id = '{$row['id']}'")->fetch_array()['total'];
					?>
						<tr>
							...
							switch ($row['status']) {
								...
								case 'Idle':
                                    echo "<span class='py-0 px-1 btn-flat btn-secondary'>Idle</span>";
                                    break;  		
								case 'Active':
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
								  	<a class="dropdown-item" href="?page=purchase_orders_poc/view_details&id=<?php echo $row['id'] ?>"><span class="fa fa-eye text-primary"></span> View</a>
				                    <div class="dropdown-divider"></div>
									
				                    <a class="dropdown-item" href="?page=purchase_orders_poc/manage_poc&id=<?php echo $row['id'] ?>"><span class="fa fa-edit text-primary"></span> Edit</a>
									<?php if($_settings->userdata('type') == 1): ?>
				                    <div class="dropdown-divider"></div>
...
<script>
	...
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this PO permanently?","delete_pocrent",[$(this).attr('data-id')])
		})
		$('.view_details').click(function(){
			uni_modal("Reservaton Details","purchase_orders_poc/view_details?id="+$(this).attr('data-id'),'mid-large')
		})
	...
	function delete_pocrent($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_pocrent",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
	...
	function renew_pocrent($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=renew_pocrent",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
