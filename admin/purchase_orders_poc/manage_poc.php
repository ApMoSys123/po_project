...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><b><u></table><?php echo isset($id) ? "Update POC Purchase Order / Letter Of Intent Details": "New POC Purchase Order" ?> </u></b></h3>
	</div>
	<div class="card-body">
				...
				<div class="col-md-12">
					...
					<table class="table table-striped table-bordered" id="item-list">
						...
						<tbody>
							<?php 
							if(isset($id)):
							$order_items_qry = $conn->query("SELECT * FROM `poc_po_list` o inner join poc_po_list i on o.id = i.id where o.`id` = '$id'  ");
							echo $conn->error;
					...
					<?php if($_settings->userdata('type') == 1): ?>
						...
						<option value="Active" <?php echo isset($status) && $status =="Active" ? "selected": "Active" ?> >Active</option>
							<option value="Idle" <?php echo isset($status) && $status == "Idle" ? 'selected': 'Idle' ?>>Idle</option>
						...
						<input type="text" class="form-control form-control-sm rounded-0"  name="project_type" value="POC" readonly></input>
	...
	<div class="card-footer">
		...
		<a class="btn btn-flat btn-default" href="?page=purchase_orders_poc">Cancel</a>
	</div>
</div>
...
//date end
	...
	function _autocomplete(_item){
		_item.find('.item_id').autocomplete({
			source:function(request, response){
				$.ajax({
					url:_base_url_+"classes/Master.php?f=search_pocitems",
					method:'POST',
					data:{q:request.term},
					dataType:'json',
					error:err=>{
	...
	$(document).ready(function(){
		...
		$('#po-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_pocpo",
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						location.href = "./?page=purchase_orders_poc/view_details&id="+resp.id;
