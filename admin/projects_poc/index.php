...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title"><b><u>POC Project Details</b></u></h3>
	<div class="card-tools">
	<?php if($_settings->userdata('type') ==  1): ?>
			<a href="?page=projects_poc/manage_pocprojects" id="create_new" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Add New </a>
			<?php endif; ?>
			...
			<?php if($_settings->userdata('type') ==  3): ?>
                <a href="?page=projects_poc/manage_pocprojects" id="create_new" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Add New </a>
			<?php endif; ?>
			<a href="http://localhost/purchase_order/admin/projects_poc/export.php" id="create_new" class="btn btn-flat btn-success"><span class="fas fa-paper-plane"></span> Export </a>
        ...
        <div class="container-fluid">
			...
			<table id="fcproject" class="table table-hover table-striped">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT pl.*, po.pname as proname , po.client_id as client_id FROM `poc_project_list` pl inner join `poc_po_list` po on pl.name = po.id order by (`name`) asc ");
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon py-0" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
				                    <a class="dropdown-item view_data" href="?page=projects_poc/view_details&id=<?php echo $row['id'] ?>" data-id = "<?php echo $row['id'] ?>"><span class="fa fa-info text-primary"></span> View</a>
				                    <div class="dropdown-divider"></div>
				                    <a class="dropdown-item edit_data" href="?page=projects_poc/manage_projectspoc&id=<?php echo $row['id'] ?>" data-id = "<?php echo $row['id'] ?>"><span class="fa fa-edit text-primary"></span> Edit</a>
				                    <div class="dropdown-divider"></div>
...
$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this Project permanently?","delete_pocproject",[$(this).attr('data-id')])
		})
		$('#create_new').click(function(){
			uni_modal("<i class='fa fa-plus'></i> Create New Project","projects_poc/manage_pocprojects.php")
		})
		$('.view_data').click(function(){
			uni_modal("<i class='fa fa-info-circle'></i> Project Details","projects_poc/view_details.php?id="+$(this).attr('data-id'),"")
		})
		$('.edit_data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Edit Project Details","projects_poc/manage_pocprojects.php?id="+$(this).attr('data-id'))
		})
		$('.table th,.table td').addClass('px-1 py-0 align-middle')
		$('.table').dataTable();
	})
	function delete_pocproject($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_pocproject",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
