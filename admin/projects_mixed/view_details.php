...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `mixed_project_list` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title"><b><u>FC Project Details</u></b> </h3>
    </div>
<div class="container fluid">
    <callout class="callout-primary">
        <dl class="row">
            ...
            <dd class="col-md-8">: 
            <?php 
                $client_qry = $conn->query("SELECT * FROM mixed_po_list where id = '{$name}'");
            ...
            <dd class="col-md-8">: 
                <?php 
                        $qry = $conn->query("SELECT po_no FROM mixed_po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
                    <?php echo $row['po_no'] ?>
            </dd>
            ...
            <dd class="col-md-8">:
            <?php 
                $client_qry = $conn->query("SELECT client_id FROM mixed_po_list where id = $name ");
                $row = $client_qry->fetch_assoc();
            ?>
            ...
            <dd class="col-md-8">: 
                <?php 
                        $qry = $conn->query("SELECT rm_apmosys FROM mixed_po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
            ...
            <dd class="col-md-8">:&nbsp;
            <?php 
                        $qry = $conn->query("SELECT project_type FROM mixed_po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
            ...
            <dd class="col-md-8">: 
               <?php 
                        $qry = $conn->query("SELECT businesshead_name FROM mixed_po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
            ...
            <dd class="col-md-8">: 
                <?php 
                 $qry = $conn->query("SELECT accountshead_name FROM mixed_po_list where id = $name ");
                 $row = $qry->fetch_assoc();
                ?>
                  <?php echo $row['accountshead_name'] ?>
                </dd>
            <dt class="col-md-4">Accounts Team Comments</dt>
            <dd class="col-md-8">: <?php echo $accountsteam_comment ?></dd>
            <?php endif; ?>
            ...
            <?php 
              ...
              $qry = $conn->query("SELECT bhead_approval FROM mixed_po_list where id = $name ");
              $row = $qry->fetch_assoc();
            ...
            <dt class="col-md-4">Accounts Head Verified</dt>
            <dd class="col-md-8">:&nbsp;
            <?php 
				switch ($acc_verified) {				
						case 'Verified':
    ...
    <table class="table table-striped table-bordered" id="item-list">
                    ...
                    <tbody>
                        ...
                        FROM mixed_po_list po where id = $name ");
                        ...
                        <tr class="po-item" data-id="">
                            ...
                            <?php 
                            $qry = $conn->query("SELECT * FROM `mixed_project_list` o inner join mixed_project_list i on o.id = i.id where o.`id` = '$id'  ");
                            $row = $qry->fetch_assoc();
                            ?>   
    ...
    <div class="row px-2 justify-content-end">
        <div class="col-1">
            ...
            <a class="btn btn-dark btn-flat btn-sm" type="button" data-dismiss="modal"  href="?page=projects_mixed">Close</a>
        </div>
    </div>
</div>
<br>