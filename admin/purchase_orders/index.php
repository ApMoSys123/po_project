...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title"><b><u>FC Purchase Order Details</b></u></h3>
		<div class="card-tools">
		<?php if($_settings->userdata('type') == 1): ?>
		<a href="?page=purchase_orders/manage_fcpo" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Add New</a>
		<?php endif; ?>
	...
	<div class="card-body">
		...
		<table id="purchaseprojectfc" class="table table-hover table-striped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * FROM `po_list`");
						while($row = $qry->fetch_assoc()):
							
								
							$row['total_amount'] = $conn->query("SELECT sum(po_value) as total FROM milestone_tablefc where po_id = '{$row['id']}'")->fetch_array()['total'];
					?>
						...
						<tr>
							...
							<?php endif; ?>
							<?php if($_settings->userdata('type') == 1): ?>
							<td class="text-center"><?php echo number_format($row['consumed']) ?></td>
							<?php endif; ?>
							<?php if($_settings->userdata('type') == 4): ?>
							<td class="text-center"><?php echo number_format($row['consumed']) ?></td>
							<?php endif; ?>
							...
							<?php endif; ?>
							<?php if($_settings->userdata('type') == 1): ?>
							<td class="text-center"><?php echo number_format($row['balance']) ?></td>
							<?php endif; ?>
							<?php if($_settings->userdata('type') == 4): ?>
							<td class="text-center"><?php echo number_format($row['balance']) ?></td>
							<?php endif; ?>
