...
<div class="container fluid">
    <callout class="callout-primary">
        <dl class="row">
            ...
            <dd class="col-md-8">: 
            <?php 
                $client_qry = $conn->query("SELECT * FROM po_list where id = '{$name}'");
            ...
            <dd class="col-md-8">: 
                <?php 
                        $qry = $conn->query("SELECT po_no FROM po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
                    <?php echo $row['po_no'] ?>
            </dd>
            ...
            <dd class="col-md-8">:
            <?php 
                $client_qry = $conn->query("SELECT client_id FROM po_list where id = $name ");
                $row = $client_qry->fetch_assoc();
            ?>
            ...
            <dd class="col-md-8">: 
                <?php 
                        $qry = $conn->query("SELECT rm_apmosys FROM po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
            ...
            <dd class="col-md-8">:&nbsp;
            <?php 
                        $qry = $conn->query("SELECT project_type FROM po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
            ...
            <dd class="col-md-8">: 
               <?php 
                        $qry = $conn->query("SELECT businesshead_name FROM po_list where id = $name ");
                        $row = $qry->fetch_assoc();
                ?>
    ...
    <table class="table table-striped table-bordered" id="item-list">
                    ...
                    <tbody>
                        ...
                        $order_items_qry = $conn->query("SELECT milestone_no,m_percent,m_description,m_startdate,m_enddate,m_status
                        FROM po_list po where id = $name ");
                        ...
                        <tr class="po-item" data-id="">
                            ...
                            <?php 
                            $qry = $conn->query("SELECT * FROM `project_list` o inner join project_list i on o.id = i.id where o.`id` = '$id'  ");
                            $row = $qry->fetch_assoc();
                            ?>   
    ...
    <div class="row px-2 justify-content-end">
        <div class="col-1">
            ...
            <a class="btn btn-dark btn-flat btn-sm" type="button" data-dismiss="modal"  href="?page=projects">Close</a>
        </div>
    </div>
</div>
<br>